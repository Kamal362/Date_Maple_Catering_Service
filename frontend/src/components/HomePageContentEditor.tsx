...
const HomePageContentEditor: React.FC<HomePageContentEditorProps> = ({ section, onClose, onSave }) => {
  ...
  const saveSection = async (sectionName: string, sectionData: Partial<HomePageContent>) => {
    try {
      const updatedContent = { ...content, ...sectionData };
  ...
  const renderFooterEditor = () => (
    <div className="space-y-6">
      ...
      <div className="border border-secondary-tea rounded-lg p-4 mt-6">
        ...
        <div className="space-y-4 max-h-64 overflow-y-auto">
          {(content.items || []).filter(item => item.role === 'hours').map((item, index) => (
            <div key={`hours-${index}`} className="border border-secondary-tea rounded-lg p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newItems = [...(content.items || [])];
                      const hoursIndex = newItems.findIndex(i => i.role === 'hours');
                      if (hoursIndex !== -1) {
                        newItems[hoursIndex] = { ...newItems[hoursIndex], name: e.target.value };
                      }
                      setContent(prev => ({ ...prev, items: newItems }));
                    }}
                    className="w-full px-3 py-2 border border-secondary-tea rounded-md text-sm"
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      const hoursIndex = newItems.findIndex(i => i.role === 'hours');
                      if (hoursIndex !== -1) {
                        newItems[hoursIndex] = { ...newItems[hoursIndex], title: e.target.value };
                      }
                      setContent(prev => ({ ...prev, items: newItems }));
                    }}
                    className="w-full px-3 py-2 border border-secondary-tea rounded-md text-sm"
          ...
          {(content.items || []).filter(item => item.role === 'hours').length === 0 && (
            <button
              type="button"
              onClick={() => {
