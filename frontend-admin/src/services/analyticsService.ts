...
export const getAnalyticsData = async (): Promise<AnalyticsData> => {
  try {
    ...
    return {
      totalRevenue: data.recentOrders?.reduce((sum: number, order: any) => sum + (order.totalAmount || 0), 0) || 0,
      totalOrders: data.stats?.orders || 0,
      totalCustomers: data.stats?.users || 0,
      totalEvents: data.stats?.events || 0,
      revenueTrend: data.revenueTrend || [],
      popularItems: [],
      orderStatusDistribution: data.orderStatusDistribution?.map((item: any) => ({
        status: item._id,
        count: item.count
      })) || [],
